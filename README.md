...
| `ask-architecture` | 성장 전제 골격(데이터 모델, 모듈 경계, 실제/스텁, 확장점)을 설계하고 평문으로 확인받습니다. |
| `ask-prompt` | 인터뷰 결과를 구현 가능한 개발 프롬프트로 정리합니다. |
| `ask-build` | 확정된 기획을 가볍게(MVP) 구현하되, 먼저 사용자 확인을 받습니다. |
| `ask-ship` | 확정된 기획을 한 번에 MVP 이상으로 구현합니다. 잘라서 만들고, 완성 바까지 채우고, 이음매와 체크리스트로 검증합니다. |
...
구현 방식을 두 갈래로 고를 수 있습니다.

- **가벼운 길**: 빠르게 MVP를 보고 싶을 때. `... -> ask-prompt -> ask-build`
- **제대로 길**: 한 번에 MVP 이상으로 뽑고 싶을 때. 기능은 최소, 각 핵심 유닛은 성장 전제 골격 위에 배포 완성도로. `... -> ask-architecture -> ask-prompt -> ask-ship`

MVP를 나중에 하나하나 고도화하기 어렵다면(바이브코딩), 제대로 길이 맞습니다. 대신 깊이를 앞에 몰수록 기획이 틀렸을 때 비용이 커지므로, 확정 전에 `ask-risk`로 점검하세요.
...
ask-plan, 어떤 스킬로 시작할지 모를 때
        ...
        -> ask-prompt
        -> 가벼운 길: ask-build
        -> 제대로 길: ask-architecture -> ask-prompt -> ask-ship
```
...
| `ask-architecture` | Design a growth-shaped skeleton (data model, seams, real-vs-stub, extension points) and confirm it in plain language. |
| `ask-prompt` | Convert interview results into an implementation-ready prompt. |
| `ask-build` | Implement an approved plan as a quick MVP, after user confirmation. |
| `ask-ship` | Implement an approved plan above MVP in one pass: slice, build each unit to a completion bar, verify seams and a checklist. |

### Two Paths

Choose how the result gets built:

- **Light path** — quick MVP output: `... -> ask-prompt -> ask-build`
- **Committed path** — above-MVP in one pass, minimal feature set with each core unit built to ship quality on a growth-shaped skeleton: `... -> ask-architecture -> ask-prompt -> ask-ship`

Pick the committed path when you cannot easily upgrade an MVP later. Front-loading depth raises the cost of a wrong plan, so run `ask-risk` before shipping.
...
ask-plan, when you are not sure where to start
        ...
        -> ask-prompt
        -> light path: ask-build
        -> committed path: ask-architecture -> ask-prompt -> ask-ship
```
